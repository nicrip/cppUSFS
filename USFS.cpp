...
{
    readRegisters(xreg, 6);  // Read the six raw data registers sequentially into data array
    x = (int16_t) (((int16_t)rbuf[1] << 8) | rbuf[0]);   // Turn the MSB and LSB into a signed 16-bit value
    y = (int16_t) (((int16_t)rbuf[3] << 8) | rbuf[2]);
    z = (int16_t) (((int16_t)rbuf[5] << 8) | rbuf[4]);
}
...
{
    readRegisters(Baro, 2);  // Read the two raw data registers sequentially into data array

    int16_t rawPressure =  (int16_t) (((int16_t)rbuf[1] << 8) | rbuf[0]);   // Turn the MSB and LSB into a signed 16-bit value
    pressure = (float)rawPressure *.01f + 1013.25f; // pressure in millibars

    // get BMP280 temperature
    readRegisters(Temp, 2);  // Read the two raw data registers sequentially into data array
    int16_t rawTemperature =  (int16_t) (((int16_t)rbuf[1] << 8) | rbuf[0]);   // Turn the MSB and LSB into a signed 16-bit value
...
void USFS::readQuaternion(float & qw, float & qx, float & qy, float &qz)
{
    ...
    qx = uint32_reg_to_float (&rbuf[0]);
    qy = uint32_reg_to_float (&rbuf[4]);
    qz = uint32_reg_to_float (&rbuf[8]);
    qw = uint32_reg_to_float (&rbuf[12]);
}
...
{
    buf[0] = data;
    if (i2c_ioctl_write(&device, subAddress, buf, 1) != 1) {
...
{
    if (i2c_ioctl_read(&device, subAddress, rbuf, count) != count) {
